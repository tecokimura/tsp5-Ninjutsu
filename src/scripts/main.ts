...
const sketch = (p: p5) => {
  ...
  function isPushKeyNow() {
    let is =
      keyCodeHistory[0] != Def.P5_KEY_NONE &&
      keyCodeHistory[1] == Def.P5_KEY_NONE
    if (is)
      Util.debug(
        "isPushKeyNow:0=" + keyCodeHistory[0] + ",1=" + keyCodeHistory[1]
      )
    return is
  }
  ...
  function isReleaseKeyNow() {
    ...
    if (is)
      Util.debug(
        "isReleaseKeyNow:0=" + keyCodeHistory[0] + ",1=" + keyCodeHistory[1]
      )
    return is
  }
  ...
  p.setup = () => {
    ...
    scene = new Scene()
    player = new Player()
    for (let i = 0; i < enemies.length; i++) enemies[i] = new Enemy()

    for (let i = 0; i < stars.length; i++) stars[i] = new Star()

    for (let i = 0; i < clouds.length; i++) clouds[i] = new Cloud()

    for (let i = 0; i < rains.length; i++) rains[i] = new Rain()

    for (let i = 0; i < snows.length; i++) snows[i] = new Snow()
  ...
  p.draw = () => {
    if (isDraw) {
      if (scene.is(Scene.INIT)) {
        drawClear()
      } else if (scene.is(Scene.LOADING)) {
        drawClear()
      } else if (scene.is(Scene.TITLE)) {
        p.push()
        drawBg()
        ...
        drawFg()
      } else if (scene.is(Scene.GAMEOVER)) {
        drawBg()
  ...
  function drawDebugInfo(p: p5) {
    if (Util.isDebugInfo) {
      ...
      p.text(
        "Camera L:" + Camera.getInLeft() + " R:" + Camera.getInRight(),
        x,
        (y += addy)
      )
      p.text(
        "Camera T:" + Camera.getInTop() + " B:" + Camera.getInBottom(),
        x,
        (y += addy)
      )
  ...
  function addEnemy() {
    if (scene.count() % 2 == 0) {
      ...
      for (let i = 1000; i < stars.length; i++) {
        ...
        {
          if (
            stars[i].add(Def.TYPE_BG_ALL, Camera.getInLeft(), Camera.getInTop())
          ) {
      ...
      for (let i = 1000; i < clouds.length; i++) {
        if (
          clouds[i].add(Def.TYPE_BG_ALL, Camera.getInLeft(), Camera.getInTop())
        ) {
      ...
      for (let i = 1000; i < rains.length; i++) {
        if (
          rains[i].add(Def.TYPE_BG_ALL, Camera.getInLeft(), Camera.getInTop())
        ) {
      ...
      for (let i = 1000; i < snows.length; i++) {
        if (
          snows[i].add(Def.TYPE_BG_ALL, Camera.getInLeft(), Camera.getInTop())
        ) {
  ...
  function drawBg(isClear: boolean = true) {
    ...
    for (i = 0; i < stars.length; i++) {
      stars[i].drawBack(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < clouds.length; i++) {
      clouds[i].drawBack(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < rains.length; i++) {
      rains[i].drawBack(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < snows.length; i++) {
      snows[i].drawBack(p, Camera.getInLeft(), Camera.getInTop())
    }
  }
  ...
  function drawFg(
    ...
    for (i = 0; i < stars.length; i++) {
      stars[i].drawFront(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < clouds.length; i++) {
      clouds[i].drawFront(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < rains.length; i++) {
      rains[i].drawFront(p, Camera.getInLeft(), Camera.getInTop())
    }
    for (i = 0; i < snows.length; i++) {
      snows[i].drawFront(p, Camera.getInLeft(), Camera.getInTop())
    }
